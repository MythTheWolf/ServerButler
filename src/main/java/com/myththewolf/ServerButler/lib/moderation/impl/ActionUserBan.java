    ...
    public ActionUserBan(String id) {
        ...
        try {
            while (resultSet.next()) {
                ...
                this.moderator = resultSet.getString("moderator") == null ? null : DataCache.getOrMakePlayer(resultSet.getString("moderator"));
    ...
    public void update() {
        if (DB_ID == null) {
            ...
            prepareAndExecuteUpdateExceptionally(SQL, 6, ActionType.BAN, reason, getTargetUser().get()
                    .getUUID(), ((getModeratorUser().isPresent() ? getModeratorUser().get()
                    .getUUID() : null)), TargetType.BUKKIT_PLAYER.toString(), TimeUtils.dateToString(dateApplied));
        } else {
            ...
            prepareAndExecuteUpdateExceptionally(SQL, 7, ActionType.BAN, reason, getTargetUser().get()
                    .getUUID(), ((getModeratorUser().isPresent() ? getModeratorUser().get()
                    .getUUID() : null)), TargetType.BUKKIT_PLAYER.toString(), Integer.parseInt(this.DB_ID), TimeUtils
