...
/**
 * This class represents a mute history entry
 */
public class ActionUserSoftmute implements ModerationAction, SQLAble {
    /**
     * The target player
     */
    ...
    /**
     * The Date and Time the mute was applied
     */
    private DateTime dateApplied;

    /**
     * Constructs a new ActionUserMute, pulling data from the Database
     *
     * @param id The ID of the entry of the database to pull data from
     */
    public ActionUserSoftmute(String id) {
        DB_ID = id;
    ...
    /**
     * Constructs a new ActionUserMute such that no entry in the database exists with these parameters
     *
     * @param reason    The reason of the ban
     * @param target    The target player
     * @param moderator The moderator, or null if from the console
     */
    public ActionUserSoftmute(String reason, MythPlayer target, MythPlayer moderator) {
        this.target = target;
    ...
    public String getReason() {
        return (reason != null ? reason : ConfigProperties.DEFAULT_MUTE_REASON);
    }
    ...
    public void update() {
        if (DB_ID == null) {
            ...
            DB_ID = Integer.toString(prepareAndExecuteUpdateExceptionally(SQL, 6, getActionType(), reason, getTargetUser().get()
